...
`tableData` 的每一行**自带 `component` 字段**（如 `"Fx[KN]"`），`parse_interval_table`
按名字把区间对到分量上：缺分量、多了不参与分组的分量、分量重复、名字不认识，
都会返回明确的中文错误。行序因此不再是隐含契约，前端改坐标系映射也不会错位。

整表都不带 `component` 时（老版本前端）退回「按位置一一对应」的老规则，
此时行数必须与分量数严格相等。这条兼容分支迟早要删，但前端是独立部署的，
exe 和网页版本可能不同步，先留着。

行里只有数字键（`"0"`~`"10"`）被当成分段边界，按列号排序后取非空值 ——
`component`、`index` 这类附加字段自动忽略。
