...
| 日志 | Loguru |
| 打包 | PyInstaller + Inno Setup（Windows 安装包） |
| 包管理 | uv |
...
项目使用 PyInstaller 打包为 Windows 可执行文件，再通过 Inno Setup 生成安装包。
...
1. **PyInstaller** → 生成 `output/simple_load/` 目录
2. **py2pyd** → 代码加密（.py → .pyd）
3. **Inno Setup** → 生成 `simple_load-{version}.exe` 安装包
